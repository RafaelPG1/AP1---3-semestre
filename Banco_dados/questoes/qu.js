  ...
  },
  {
    ...
    "questions": [
      ...
      },
      {
        ...
        "answer": 1,
        "feedback": "✓ Resposta correta: B) T-SQL (Transaction SQL)\n\n<strong>Por que está certa:</strong> O SQL Server da Microsoft utiliza o T-SQL (Transact-SQL) como sua linguagem procedural proprietária. O T-SQL estende o SQL padrão com suporte a variáveis, estruturas de controle de fluxo (IF/ELSE, WHILE), tratamento de erros (TRY/CATCH) e procedimentos armazenados. É importante não confundir com PL/SQL, que é a linguagem procedural do Oracle — um erro comum entre iniciantes, exatamente o citado no enunciado."
      },
      {
  ...
  },
  {
    ...
    "questions": [
      ...
      },
      {
        ...
        "answer": 3,
        "feedback": "✓ Resposta correta: D) Atomicidade\n\n<strong>Por que está certa:</strong> A Atomicidade é a propriedade ACID que garante que uma transação seja tratada como uma unidade indivisível — ou todas as operações são executadas com sucesso (COMMIT), ou nenhuma delas é aplicada (ROLLBACK). No caso da transferência, o débito na conta de Pedro e o crédito na conta de Maria devem ocorrer juntos ou nenhum dos dois. Como houve falha no meio da transação, o sistema deve desfazer o débito de Pedro, restaurando o estado consistente anterior."
      },
      {
  ...
  },
  {
    ...
    "questions": [
      ...
      },
      {
        ...
        "answer": 2,
        "feedback": "✓ Resposta correta: C) Fragmentação Vertical\n\n<strong>Por que está certa:</strong> A Fragmentação Vertical divide uma tabela por colunas (atributos), distribuindo grupos de colunas em diferentes servidores. No cenário, as colunas Saldo e Limite ficaram no servidor de Auditoria, enquanto Nome e Endereço ficaram no servidor de Marketing — ambos os fragmentos mantêm a chave primária para permitir a junção. Em contraste, a Fragmentação Horizontal divide uma tabela por linhas (tuplas), distribuindo subconjuntos de registros em diferentes servidores."
      },
      {
        ...
        "answer": 3,
        "feedback": "✓ Resposta correta: D) Camada Intermediária (Servidor de Aplicação)\n\n<strong>Por que está certa:</strong> Na arquitetura de 3 camadas, a Camada Intermediária (ou Camada de Negócio) é o Servidor de Aplicação, responsável por processar as regras de negócio — como validação de crédito, cálculo de frete e processamento de pagamento. Como o banco de dados (camada 3) estava normal e os usuários conseguiam navegar (camada 1 funcionando), o gargalo estava claramente na camada do meio, que processa as transações de pagamento e estava sobrecarregada com o volume da Black Friday."
      },
      {
      ...
      },
      {
        ...
        "answer": 1,
        "feedback": "✓ Resposta correta: B) Computação em Nuvem (Cloud)\n\n<strong>Por que está certa:</strong> A Computação em Nuvem apresenta desafios específicos de segurança e conformidade legal: os dados ficam armazenados em servidores de terceiros (o provedor de nuvem), e a empresa perde o controle físico direto sobre a infraestrutura. Isso gera preocupações com: soberania dos dados, conformidade com leis como a LGPD (especialmente em caso de vazamentos), dependência do provedor e riscos de acesso não autorizado. Esses são trade-offs conhecidos e documentados da adoção de soluções cloud."
      },
      {
        ...
        "answer": 2,
        "feedback": "✓ Resposta correta: C) O Servidor de Aplicação (ou Web)\n\n<strong>Por que está certa:</strong> Na arquitetura de 3 camadas, o Servidor de Aplicação (ou Servidor Web) é a camada intermediária responsável por centralizar a lógica de negócio. Ele recebe as requisições dos usuários (camada de apresentação), aplica as regras de negócio (verificação de prazos, pré-requisitos, validações), e só então interage com o banco de dados (camada de dados) para executar as operações necessárias. Isso garante segurança, manutenibilidade e escalabilidade do sistema."
      },
      {
  ...
  },
  {
    ...
    "questions": [
      ...
      },
      {
        ...
        "answer": 1,
        "feedback": "✓ Resposta correta: B) Integridade Referencial\n\n<strong>Por que está certa:</strong> A Integridade Referencial garante que todo valor de chave estrangeira em uma tabela filho deve referenciar um valor existente na chave primária da tabela pai. O campo CodAlunoDisciplina na tabela MATRICULA é uma chave estrangeira que referencia CodAluno na tabela ALUNOS. Ao tentar inserir o valor 99, que não existe em ALUNOS, o SGBD detecta a violação e rejeita a operação — pois não pode existir uma matrícula referenciando um aluno inexistente (registro órfão)."
      },
      {
  ...
  },
  {
    ...
    "questions": [
      ...
      },
      {
        ...
        "answer": 2,
        "feedback": "✓ Resposta correta: C) Há pelo menos um objeto envolvido no relacionamento\n\n<strong>Por que está certa:</strong> Na notação UML de multiplicidade: '0..*' significa zero ou mais (opcional, sem limite superior); '1..*' significa um ou mais (obrigatório pelo menos um, sem limite superior); '1..1' ou simplesmente '1' significa exatamente um; '0..1' significa zero ou um (opcional, no máximo um). A notação '1..*' ao lado de ItemDePedido indica que cada Pedido deve ter pelo menos 1 item, podendo ter muitos — um pedido sem nenhum item viola essa restrição de multiplicidade."
      },
      {
      ...
      },
      {
        ...
        "answer": 0,
        "feedback": "✓ Resposta correta: A) 1..1 e 0..*\n\n<strong>Por que está certa:</strong> Analisando as regras de negócio: do lado de Turma (quantas turmas um Aluno pode ter): cada aluno pertence a exatamente uma turma, nem mais nem menos → multiplicidade 1..1; do lado de Aluno (quantos alunos uma Turma pode ter): uma turma pode ter zero alunos (turma recém-criada) ou muitos → multiplicidade 0..*. Portanto, a leitura é: um Aluno pertence a exatamente 1 Turma (1..1), e uma Turma pode ter de 0 a muitos Alunos (0..*)."
      }
    ]
  },
  {
...
function buildFeedbackHTML(question, isCorrect) {
    ...
    const body = cleanFeedback.replace(/\n/g, '<br>');
...
function ativarModoStep() {
    ...
    if (primeiraUnanswered !== -1 && currentQuestion === 0) {
        currentQuestion = primeiraUnanswered;
    }
...
function ativarModoScroll() {
    ...
    document.querySelector('.page-wrapper')?.classList.remove('modo-step-wrapper');
