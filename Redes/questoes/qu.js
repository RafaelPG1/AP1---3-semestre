...
function createShuffledQuizData() {
    return originalQuizData.map(subject => ({
        ...
        questions: subject.questions.map(question => {
            ...
            const explanationMatch = originalFeedback.match(/Por que está certa:\s*([\s\S]*)/);
        ...
        subject.questions.forEach(question => {
            questionsHTML += `
                <div class="question-container">
                    ...
                    ${question.texto ? `<div class="question-text question-texto">${question.texto}</div>` : ''}
                    <div class="question-text question-enunciado">${question.question}</div>
                    ...
                    ${quizSubmitted ? `
                        <div class="feedback ${userAnswers[questionIndex] === question.answer || userAnswers[questionIndex] === null ? 'correct-feedback' : 'incorrect-feedback'}">
                            <pre style="white-space: pre-wrap; font-family: inherit; margin: 0;">${question.feedback}</pre>
                        </div>` : ''}
                </div>`;
            questionIndex++;
        });
